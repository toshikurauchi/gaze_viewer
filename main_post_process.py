...
def load_data(filename, dtype):
    with open(filename) as f:
        ...
        return [dict(**row, dtype=dtype, frame_idx=i) for i, row in enumerate(reader)]


def process(screen_filename, overlayed_filename, screen_data, data):
    ...
    for d in tqdm(data):
        ...
        if dtype == SCREEN_TYPE:
            ...
            if writer is None:
                ...
                writer = VideoWriter(overlayed_filename, width, height, 20.0)
...
def postprocess(recording):
    ...
    process(screen_filename, overlayed_filename, screen_data, data)
